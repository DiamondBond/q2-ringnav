...
- The key tone is off by default, so wheel turns and button presses are silent. Turn **Key Tone** on in the system settings if you want the stock feedback clicks.
...
`release/bin/demo`, the boot logo and the default `release/assets/default/raw/config.ini` inside `rootfs.squashfs` change. The kernel is byte-identical, and the builder checks every other inode's name, type, mtime, mode, uid and gid against stock.
...
The shipped default config sets `KEYTONE=0`, so a fresh install has the key tone disabled. The app copies that file to `/mnt/data/config.ini` only when no user copy exists; an already-used device keeps its saved setting, and the setting can be changed at any time in the system settings.
...
For on-device acceptance, browse a long list and confirm immediate response, an unobtrusive transition to two- and three-row steps and precise reversal. Tap and swipe across panes and pages and confirm no outline lingers or returns until wheel/centre input; also try a wheel turn at a list end. Tune `ACCEL2_HOLD_MS`, `ACCEL3_HOLD_MS` and `ACCEL_MS` if needed. Check the selected-row margin in ordinary and music lists, including their ends and tall rows; spin the home wheel and confirm rapid turns visibly advance through multiple icons, reverse midway through both slow and fast slides, and check that no unwanted movement remains after the final slide settles. Tap a row as soon as a list appears, and again while a long list is still settling, and confirm the touched row opens. Also browse parent → child → grandchild folders, return to each selected folder, and check quick turns on short menus. Also revisit albums/queries and confirm their selections remain separate. Emulator checks do not replace this hardware check.
