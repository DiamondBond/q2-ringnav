...
5. Check **About** and make sure it shows `V2.8R`.
...
- Turn the scroll wheel to move through menu items; vertical lists update immediately, one row per accepted tick. Lists longer than 16 rows step two rows after 300 ms and three rows after 600 ms of continuous same-direction ticks no more than 140 ms apart. Pausing, reversing or reaching an end returns to one-row steps.
- Pressing return, play/pause, prev/next or the centre button ticks once. The capacitive touch of your finger no longer adds a second tick, and the wheel is ready again as soon as the button is released. Each home wheel tick accepted by the stock input filter advances one icon: isolated ticks use a 200 ms slide, and consecutive same-direction ticks within 200 ms use 120 ms slides. Reversing immediately heads back from the current visual position at normal speed, canceling the unfinished destination.
...
During wheel/centre use, the selected item gets a single rounded white outline seated on a dark separator line, over a subtle dark fill, so it stays readable over bright album art. The home screen keeps its normal selected-card look without the extra outline.
...
| `on_wm_keydown_before_fun` | `0x4e8424` | Hold wheel detents and drop capacitive crosstalk             |
...
Lists with at most `SHORT_LIST_MAX` (16) logical entries always move one row per detent. This named constant in `patch/ringnav.c` can be tuned for the hardware; virtual tables use their total logical count, not the recycled row pool size. Longer lists use three speeds: one row per accepted tick, two after `ACCEL2_HOLD_MS` (300 ms) of continuous same-direction ticks spaced at most `ACCEL_MS` (140 ms) apart, and three after `ACCEL3_HOLD_MS` (600 ms). These named constants remain available for hardware tuning; these timings are provisional until checked on the Q2. A pause, reversal, touch, native click, centre press, list boundary or rejected navigation resets the run. Acceleration belongs to the current window, pane and browsing scope; a list resize also resets it. Wheel movement cancels existing momentum and uses the stock `scroll_view_set_offset` / `table_client_set_yoffset` setters immediately, reloading table row references after synchronous rebinding. Remembered-position restoration retains its separate glide. This follows the row-based approach of Rockbox's [H2/Eros Q wheel mapping](https://github.com/Rockbox/rockbox/blob/master/apps/keymaps/keymap-erosq.c) and [list implementation](https://github.com/Rockbox/rockbox/blob/master/apps/gui/list.c). Selected and restored rows use a `SCROLL_MARGIN` of 12 pixels above and below, reduced to half the spare viewport height in tight lists and clamped at content boundaries. Rows taller than the viewport align their top edge when selected or restored, keeping their title visible.

The click wheel's capacitive touch and its buttons are independent input sources, so pressing a button also delivers a phantom wheel detent. Wheel key-downs are held for `WHEEL_SETTLE_MS` (25 ms): a button key-down inside that window drops the detent before it clicks or moves the list, leaving the button's own tick as the only one. A button that arrives just after the detent was accepted does not tick again (`WHEEL_PAIR_MS`, 120 ms). The stock lockout byte at `0xa37c89` — written to 8 by a return, play/pause, centre, 222 or 223 key-down, decremented every 100 ms, and honoured by the stock key-up filter — is read before holding a detent and cleared at every button release, so the wheel is not left unresponsive for up to 800 ms. Both constants can be tuned for the hardware.
...
For on-device acceptance, browse a long list and confirm immediate response, an unobtrusive transition to two- and three-row steps and precise reversal. Press each wheel button and confirm a single tick with no stray step, then spin immediately after and confirm the wheel responds. Tap and swipe across panes and pages and confirm no outline lingers or returns until wheel/centre input; also try a wheel turn at a list end. Tune `ACCEL_HOLD_MS` and `ACCEL_MS` if needed. Check the selected-row margin in ordinary and music lists, including their ends and tall rows; spin the home wheel and confirm rapid turns visibly advance through multiple icons, reverse midway through both slow and fast slides, and check that no unwanted movement remains after the final slide settles. Tap a row as soon as a list appears, and again while a long list is still settling, and confirm the touched row opens. Also browse parent → child → grandchild folders, return to each selected folder, and check quick turns on short menus. Also revisit albums/queries and confirm their selections remain separate. Emulator checks do not replace this hardware check.
